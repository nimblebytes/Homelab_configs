# Proxmox post installation task
Task to perform after a new Proxmox Install

- [Proxmox post installation task](#proxmox-post-installation-task)
- [1. Proxmox Updates](#1-proxmox-updates)
  - [Enable updated for Proxmox](#enable-updated-for-proxmox)
  - [Disable Proxmox Enterprise updates](#disable-proxmox-enterprise-updates)
    - [Disable Subscription Notification](#disable-subscription-notification)
  - [Additional hardening / encryption measures](#additional-hardening--encryption-measures)
- [2. Setup Storage Options](#2-setup-storage-options)
  - [(Optional) Remove partition](#optional-remove-partition)
  - [Create the ZFS Pool](#create-the-zfs-pool)
- [3. SMART monitoring](#3-smart-monitoring)
- [4. Enable PCI passthrough (IOMMU)](#4-enable-pci-passthrough-iommu)
  - [4.1 Configure BIOS](#41-configure-bios)
  - [4.2 Update bootloader config ()](#42-update-bootloader-config-)
    - [4.2.a For GRUB Bootloader](#42a-for-grub-bootloader)
    - [4.2.b. For Systemd/ZFS Bootloader](#42b-for-systemdzfs-bootloader)
  - [4.3 Load Kernel Modules for IOMMU](#43-load-kernel-modules-for-iommu)
- [5. Configure Network and VLANs](#5-configure-network-and-vlans)
  - [5.1. VLAN aware bridge](#51-vlan-aware-bridge)
    - [5.1.1 VLANs: Interface, Bridge, VM](#511-vlans-interface-bridge-vm)
    - [5.1.2 Interface VLAN setup](#512-interface-vlan-setup)
  - [5.2 (Optional) Isolate / move management interface](#52-optional-isolate--move-management-interface)
  - [5.3. (Optional) Network Interface bonding](#53-optional-network-interface-bonding)
- [6. Setup NFS Shares](#6-setup-nfs-shares)
- [Optional improvements](#optional-improvements)
  - [Optimal VM Setting](#optimal-vm-setting)
  - [Disable pointer for headless / non-desktop servers](#disable-pointer-for-headless--non-desktop-servers)
  - [Create templates for common VM settings](#create-templates-for-common-vm-settings)
  - [Disable file access logging for ZFS installs](#disable-file-access-logging-for-zfs-installs)
- [7. Create Backups](#7-create-backups)
- [8. Define what VLAN are needed - check todie first if not already there](#8-define-what-vlan-are-needed---check-todie-first-if-not-already-there)


# 1. Proxmox Updates

## Enable updated for Proxmox
...
## Disable Proxmox Enterprise updates
...
### Disable Subscription Notification
...
## Additional hardening / encryption measures
The following measures need to be investigated and in needed, intergrated into this guide.
[Proxmox - Hardening](https://dustri.org/b/hardening-proxmox-against-physical-attacks.html)

# 2. Setup Storage Options
...
# 4. Enable PCI passthrough (IOMMU)  
...
## 4.2 Update bootloader config (![Proxmox Bootloader](https://pve.proxmox.com/wiki/Host_Bootloader#sysboot_determine_bootloader_used))
...
---
### 4.2.b. For Systemd/ZFS Bootloader
...
---
## 4.3 Load Kernel Modules for IOMMU
...
# 5. Configure Network and VLANs

## 5.1. VLAN aware bridge
...
### 5.1.1 VLANs: Interface, Bridge, VM
...
Each have their own benefits or disadvantages:
1. Any VM attached to this bridge will automatically be tagged with the VLAN. Thus, it is not necessary to assign an VLAN ID to the VM, nor remember which ID is necessary. The disadvantage is that the VLAN is fixed and to use multiple VLAN in the VM will require.[^1]multiple interfaces and bridges to be passed to the VM.
2. Unable to find a valid usecase for such a setup (not recommended)
3. Each VM is individualy assigned a VLAN, hence the bridge works like a L3 switch. Multiple VLANs can then be processed
...
## 5.2 (Optional) Isolate / move management interface
...
## 5.3. (Optional) Network Interface bonding 
...
## Disable pointer for headless / non-desktop servers
...
## Create templates for common VM settings
...
## Disable file access logging for ZFS installs
...
# 8. Define what VLAN are needed - check todie first if not already there
