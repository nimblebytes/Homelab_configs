...
${SCRIPT_DIR}/download_cloud_vm_image.sh -S -d13
if [ $? -ne 0 ]; then
  printf "Error ocurred running script to download and customize OS image. Aborting.\n"
...
DEF_OS_IMAGE=$(./download_cloud_vm_image.sh -m -d13)
...
if [ $? -ne 0 ]; then
  printf "Error ocurred running script to create the VM. Aborting.\n"
...
qm start ${DEF_VM_ID}
if [ $? -ne 0 ]; then
  printf "Error ocurred starting the VM.\n"
